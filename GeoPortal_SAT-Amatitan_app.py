...
def agregar_capa_ee(mapa, ee_image, vis_params, nombre, opacity=1.0):
    ...
    # Si hay parámetros de visualización (como paletas), "quemamos" los colores en una imagen RGB
    if vis_params and ("palette" in vis_params or "min" in vis_params):
        img_render = img_render.visualize(**vis_params)
        
    # Al pedir el mapId de una imagen ya visualizada (RGB), se evitan los cálculos complejos de teselas
